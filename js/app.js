...
<div class="exercise" style="padding:0; border:none; background:transparent;">
<div id="hallucination-quiz-container"></div>
</div>
</div>
</div>
...
`;
    ...
    setTimeout(() => {
        ...
        window.handleQuiz = function(type, selected, btnEl) {
            ...
            const idx = type === 'hallucination' ? window.state.hIndex : window.state.dIndex;
            const q = data[idx];
            const isCorrect = (selected === q.ans);
            const category = type === 'hallucination' ? "Hallucinations (3)" : "Data Safety (15)";

            if(isCorrect) window.state.scores[category].correct++;
        ...
        window.nextQuiz = function(type) {
            if(type === 'hallucination') { window.state.hIndex++; renderH(); }
            else { window.state.dIndex++; renderD(); }
            checkResults();
        };
        ...
        function checkResults() {
            if(window.state.hIndex >= hData.length && window.state.dIndex >= dData.length) {
                const el = document.getElementById('m3-results-container');
                if(!el) return;
                
                const hc = window.state.scores["Hallucinations (3)"].correct;
                const ht = window.state.scores["Hallucinations (3)"].total;
                const dc = window.state.scores["Data Safety (15)"].correct;
                const dt = window.state.scores["Data Safety (15)"].total;
